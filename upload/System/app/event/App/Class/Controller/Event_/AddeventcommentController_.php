...
class Addeventcomment_C_Controller extends InitController{
	...
	public function index(){
		...
		// 读取活动
		$oEvent=EventModel::F('event_id=? AND event_status=1',intval(Q::G('event_id')))->getOne();
		if(empty($oEvent['event_id'])){
			$this->E(Q::L('活动不存在','Controller'));
		}
		...
		// 评论名字检测
		$sCommentName=trim(Q::G('eventcomment_name'));
		...
		// 评论内容长度检测
		$sCommentContent=C::cleanJs(strip_tags(trim(Q::G('eventcomment_content'))));
		...
		if($oEventcomment->isError()){
			...
			$oEventTemp->updateEventcommentnum(intval(Q::G('event_id')));
			...
			try{
				Comment_Extend::addFeed(Q::L('评论了活动','Controller'),'addeventcomment',$sCommentLink,$sCommentTitle,$sCommentMessage);
			}catch(Exception $e){
				$this->E($e->getMessage());
			}
			...
			if($oEvent['user_id']!=$GLOBALS['___login___']['user_id']){
				...
				try{
					Comment_Extend::addNotice(Q::L('评论了活动','Controller'),'addeventcomment',$sCommentLink,$sCommentTitle,$sCommentMessage,$oEvent['user_id'],'addeventcomment',$oEvent['event_id']);
				}catch(Exception $e){
					$this->E($e->getMessage());
				}
			}
			...
			if($arrParsecontent['atuserids']){
				foreach($arrParsecontent['atuserids'] as $nAtuserid){
					if($nAtuserid!=$GLOBALS['___login___']['user_id']){
						...
						$sNoticetemplate='<div class="notice_credit"><span class="notice_title"><a href="{@space_link}">{user_name}</a>&nbsp;'.Q::L('在活动评论中提到了你','Controller').'</span><div class="notice_content"><div class="notice_quote"><span class="notice_quoteinfo">{content_message}</span></div></div><div class="notice_action"><a href="{@eventcomment_link}">'.Q::L('查看','Controller').'</a></div></div>';

						$arrNoticedata=array(
							'@space_link'=>'home://space@?id='.$GLOBALS['___login___']['user_id'],
							'user_name'=>$GLOBALS['___login___']['user_name'],
							'@eventcomment_link'=>$sCommentLink,
		...
		$arrCommentData['jumpurl']='event://e@?id='.$oEventcomment->event_id.'&isolation_commentid='.$oEventcomment['eventcomment_id']).
				'#comment-'.$oEventcomment['eventcomment_id'];

		// 更新积分
		Core_Extend::updateCreditByAction('commoncomment',$GLOBALS['___login___']['user_id']);

		$this->A($arrCommentData,Q::L('添加活动评论成功','Controller'),1);
	}
