...
class Prev_C_Controller extends InitController{
	...
	public function index(){
		...
		// 读取小组中的上一个帖子
		$arrGrouptopicprev=Model::F_('grouptopic','@A','A.grouptopic_id<? AND A.grouptopic_status=1 AND A.group_id=?',$nId,$arrGrouptopic['group_id'])
			->setColumns('A.grouptopic_id')
			->order('A.grouptopic_id DESC')
			->getOne();
		if(empty($arrGrouptopicprev['grouptopic_id'])){
			$this->E(Q::L('没有比这更早的主题了','Controller'));
		}

		C::urlGo(Group_Extend::getTopicurl($arrGrouptopicprev));
		exit();
	}
