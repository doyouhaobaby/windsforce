...
class Hidecomment_C_Controller extends InitController{
	...
	public function index(){
		...
		if(!Groupadmin_Extend::checkCommentadminRbac($arrGrouptopic,array('group@grouptopicadmin@hidecomment'))){
			$this->E(Q::L('你没有权限屏蔽或者显示回帖','Controller'));
		}
		...
		foreach($arrGrouptopiccomments as $nGrouptopiccomment){
			...
			if(!empty($oGrouptopiccomment['grouptopiccomment_id']) && $oGrouptopiccomment->grouptopiccomment_ishide!=$nStatus){
				...
				if($GLOBALS['___login___']['user_id']!=$oGrouptopiccomment['user_id']){
					$sNoticetemplate='<div class="notice_'.($nStatus==1?'hidereplycomment':'showreplycomment').'"><span class="notice_title"><a href="{@space_link}">{user_name}</a>&nbsp;'.($nStatus==1?Q::L('对你的回帖执行了屏蔽','Controller'):Q::L('对你的回帖执行了显示','Controller')).'</span><div class="notice_content"><div class="notice_quote"><span class="notice_quoteinfo">{admin_reason}</span></div>&nbsp;'.Q::L('如果你对该操作有任何疑问，可以联系相关人员咨询','Controller').'</div><div class="notice_action"><a href="{@grouptopiccomment_link}">'.Q::L('查看','Controller').'</a></div></div>';
		...
		$this->A(array('group_id'=>$nGroupid,'grouptopic_url'=>$sGrouptopicurl),$nStatus==1?Q::L('屏蔽回帖成功','Controller'):Q::L('取消屏蔽回帖成功','Controller'));
	}
