...
class Closetopic_C_Controller extends InitController{
	...
	public function index(){
		...
		if(!Groupadmin_Extend::checkTopicadminRbac($nGroupid,array('group@grouptopicadmin@closetopic'))){
			$this->E(Q::L('你没有关闭或者打开帖子的权限','Controller'));
		}
		...
		foreach($arrGrouptopics as $nGrouptopic){
			...
			if(!empty($oGrouptopic['grouptopic_id']) && $oGrouptopic->grouptopic_isclose!=$nStatus){
				...
				if($GLOBALS['___login___']['user_id']!=$oGrouptopic['user_id']){
					$sNoticetemplate='<div class="notice_'.($nStatus==0?'opengrouptopic':'closegrouptopic').'"><span class="notice_title"><a href="{@space_link}">{user_name}</a>&nbsp;'.($nStatus==1?Q::L('对你的主题执行了关闭','Controller'):Q::L('对你的主题执行了打开','Controller')).'&nbsp;<a href="{@grouptopic_link}">'.$oGrouptopic['grouptopic_title'].'</a></span><div class="notice_content"><div class="notice_quote"><span class="notice_quoteinfo">{admin_reason}</span></div>&nbsp;'.Q::L('如果你对该操作有任何疑问，可以联系相关人员咨询','Controller').'</div><div class="notice_action"><a href="{@grouptopic_link}">'.Q::L('查看','Controller').'</a></div></div>';
					$arrNoticedata=array(
		...
		$this->A(array('group_id'=>$nGroupid),$nStatus==0?Q::L('打开主题成功','Controller'):Q::L('关闭主题成功','Controller'));
	}
