...
class Recommendtopic_C_Controller extends InitController{
	...
	public function index(){
		...
		if(!Groupadmin_Extend::checkTopicadminRbac($nGroupid,array('group@grouptopicadmin@recommendtopic'))){
			$this->E(Q::L('你没有推荐或者取消推荐帖子的权限','Controller'));
		}
		...
		foreach($arrGrouptopics as $nGrouptopic){
			...
			if(!empty($oGrouptopic['grouptopic_id']) && $oGrouptopic->grouptopic_isrecommend!=$nStatus){
				...
				if($GLOBALS['___login___']['user_id']!=$oGrouptopic['user_id']){
					$sNoticetemplate='<div class="notice_'.($nStatus==1?'recommendgrouptopic':'unrecommendgrouptopic').'"><span class="notice_title"><a href="{@space_link}">{user_name}</a>&nbsp;'.($nStatus==1?Q::L('对你的主题执行了推荐','Controller'):Q::L('对你的主题执行了取消推荐','Controller')).'&nbsp;<a href="{@grouptopic_link}">'.$oGrouptopic['grouptopic_title'].'</a></span><div class="notice_content"><div class="notice_quote"><span class="notice_quoteinfo">{admin_reason}</span></div>&nbsp;'.Q::L('如果你对该操作有任何疑问，可以联系相关人员咨询','Controller').'</div><div class="notice_action"><a href="{@grouptopic_link}">'.Q::L('查看','Controller').'</a></div></div>';
		...
		$this->A(array('group_id'=>$nGroupid),$nStatus==1?Q::L('推荐主题成功','Controller'):Q::L('取消推荐主题成功','Controller'));
	}
